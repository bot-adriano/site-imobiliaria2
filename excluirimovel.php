...
<body>
    ...
    <section id="main">
        ...
        if (isset($_GET["f_bt_excluir_imovel"])) {
            $vid = $_GET["f_imovel"];/*aqui o id do colaborador que quero excluir*/
            /*seleção de imagens do imovel*/
            $sql = "SELECT * FROM tb_casa WHERE id_casa='$vid'";
            $res = mysqli_query($con, $sql);
            while ($exibe = mysqli_fetch_assoc($res)) {
                $fotos = array($exibe["foto1"], $exibe["foto2"], $exibe["foto3"]);
                $mini = array($exibe["mini1"], $exibe["mini2"], $exibe["mini3"]);
            }
            $sql = "DELETE FROM tb_casa WHERE id_casa=$vid";/*colocamos o coamndo delete da tabela colaboradores quando quando o id colaborador for igual ao valor da variavel id*/
        ...
        <form name="f_excluir_imovel" action="excluirimovel.php" class="f_colaborador" method="get">
            ...
            <select name="f_imovel" size="10">
                ...
                $sql = "SELECT tb_casa.*,tb_imoveis.*,tb_tipo_imovel.* FROM tb_casa INNER JOIN tb_imoveis ON  tb_casa.id_qualimovel = tb_imoveis.id_imovel INNER JOIN tb_tipo_imovel on tb_casa.id_modelocasa = tb_tipo_imovel.id_tipo_imovel";
                $res = mysqli_query($con, $sql);
                // $sql = "SELECT * FROM tb_casa";/*rotina php com comando sql pra mostrar todos os colaboradoes da tabela usuario*/
                ...
                while ($exibe = mysqli_fetch_array($col)) {/*mysqli_fetch array para retornar em forma de array o total de colaboradores e while para para escrever um option enquanto estiver retornando linhas pelo fetch_array*/
                    echo "<option value='" . $exibe['id_casa'] . "'>" . $exibe['imovel'] . "-" . $exibe['id_casa'] . "-" . $exibe['tipo'] . "-" . $exibe['id_rua'] . "-" . $exibe['venda'] . "-" . $exibe['aluguel'] . "</option>";/*na variavel exibe podemos colocar o indice 0 tb (que é o numero dele dentro do sql) e escreve o nome no lugar do id*/
                }
                ?>
            </select>

            <input type="submit" name="f_bt_excluir_imovel" class="btmenu" value="excluir">
            <!--botao para gravar-->
